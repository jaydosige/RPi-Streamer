...
class Player:
    ...
    def _build_command(self, cfg: config.Config, mode: str, target: str) -> List[str]:
        ...
        # cable being out.
        # Only when a connector exists but advertises nothing, which is the
        # cable-out case. No connectors at all means no DRM on this machine,
        # where a missing backend is the more useful thing to report.
        conn = display.pick_connector(cfg.connector)
        if conn is not None and not conn.modes:
            raise RuntimeError(
                f"{conn.name} reports no modes — nothing is plugged in. Either "
                f"connect a display, or run headless by adding "
                f"video=HDMI-A-1:1920x1080@60e to /boot/firmware/cmdline.txt "
                f"and rebooting."
            )
        if mode == MODE_IDLE:
